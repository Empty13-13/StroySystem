...
if(document.querySelector('.photos__swiper')){
    ...
    autoplay: {
      ...
      disableOnInteraction: false,
		...
		// Dotts
		pagination: {
			el: '.swiper-photos__dotts',
			clickable: true,
		},
		// Arrows
		navigation: {
			nextEl: '.photos-arrow_next',
			prevEl: '.photos-arrow_prev',
		},
    breakpoints: {
      320: {
        slidesPerView: 1,
...
function selects_close(e) {
  ...
  if (!e.target.closest('.select') && !e.target.classList.contains('_option')) {
    for (let index = 0; index < selects.length; index++) {
      ...
      selectTitle.classList.remove('_active');
...
function select_init(select) {
  ...
  select_parent.insertAdjacentHTML('beforeend', '<div class="select select_' + select_modifikator + '"  tabindex="' + select_tabindex + '"></div>');
...
function _goto(target_block, speed, offset = 0) {
  ...
  // 	header = 'header';
  // }
  let menuHeight = document.querySelector('.header__wrapper').offsetHeight;

  let options = {
    speedAsDuration: true,
    speed: speed,
    header: header,
    offset: offset + menuHeight,
    easing: 'easeOutQuad',
  };
